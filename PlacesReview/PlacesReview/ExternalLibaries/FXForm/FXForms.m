...
@implementation FXFormRatingCell
...
- (void)setUp
{
  ...
  [ratingView addTarget:self action:@selector(valueChanged) forControlEvents:UIControlEventValueChanged];
...
- (void)layoutSubviews
{
  ...
  CGRect textFieldFrame = self.ratingView.frame;
  textFieldFrame.origin.x = self.textLabel.frame.origin.x + MAX(FXFormFieldMinLabelWidth, self.textLabel.frame.size.width) + FXFormFieldLabelSpacing;
  textFieldFrame.origin.y = (self.contentView.bounds.size.height - textFieldFrame.size.height) / 2;
  textFieldFrame.size.width = self.ratingView.superview.frame.size.width - textFieldFrame.origin.x - FXFormFieldPaddingRight;
  if (![self.textLabel.text length])
  {
    textFieldFrame.origin.x = FXFormFieldPaddingLeft;
    textFieldFrame.size.width = self.contentView.bounds.size.width - FXFormFieldPaddingLeft - FXFormFieldPaddingRight;
  ...
  self.ratingView.frame = textFieldFrame;
...
{
  self.textLabel.text = self.field.title;
  self.ratingView.value = [self.field.value doubleValue];
}

- (void)valueChanged
{
  self.field.value = @(self.ratingView.value);
